...
class MonitorPolicyTests(unittest.TestCase):
    ...
    def test_menu_uses_policy_without_spawning_settings_refreshes_on_focus(self):
        ...
        self.assertIn("function flyoutLaneWidthFor(screen, effectiveConnectorWidth)", menu)
        self.assertIn("if (!sidebarVisibleOnScreen(screen)) return 0", menu)
        self.assertIn("function maxFlyoutExtentFor(screen)", menu)
        self.assertIn("maxFlyoutExtentFor(screen)", menu)
