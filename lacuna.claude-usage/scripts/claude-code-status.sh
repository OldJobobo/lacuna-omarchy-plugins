...
idle_payload() {
  printf '%s\n' '{"text":"idle","tooltip":"<b>Claude Code</b><br/>No active 5-hour usage block","class":"normal"}'
}

normalize_output() {
  if [[ "$1" == '{"text":"","tooltip":"","class":"hidden"}' ]]; then
    idle_payload
  else
    printf '%s\n' "$1"
  fi
...
emit_cache_if_fresh() {
  ...
  normalize_output "$(cat "$cache_file")"
  exit 0
}
